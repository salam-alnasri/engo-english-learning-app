import 'dart:math';
import 'package:engo/data/a2_data.dart';
...
class A1Q3Controller extends GetxController {
  final List<Map<String, String>> data = a2data;
  final _rnd = Random();
  ...
  Future<void> initProgress() async {
    ...
    correctCount.value = prefs.getInt('a3_correct') ?? 0;
    wrongCount.value = prefs.getInt('a3_wrong') ?? 0;
    List<String> doneIndexes = prefs.getStringList('a1q3_doneIndexes') ?? [];
    ...
    if (remainingIndexes.isEmpty) {
      ...
      wrongCount.value = 0;
      remainingIndexes = List.generate(data.length, (i) => i);
      await prefs.remove('a3_doneIndexes');
    }
  ...
  Future<void> saveProgress() async {
    final prefs = await SharedPreferences.getInstance();
    prefs.setInt('a3_correct', correctCount.value);
    prefs.setInt('a3_wrong', wrongCount.value);
    final doneIndexes = List.generate(data.length, (i) => i)
        .where((i) => !remainingIndexes.contains(i))
        .map((i) => i.toString())
        .toList();
    prefs.setStringList('a3_doneIndexes', doneIndexes);
  }
...
class A3 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<A1Q3Controller>(
      init: A1Q3Controller(),
      builder: (c) {
        ...
        final arText = c.data[c.qIndex]["ar"]!;
        ...
        return Scaffold(
          appBar: AppBar(
            title: const Text('اختبار معرفة الجمل الإنكليزية'),
                  ...
                  Card(
                    ...
                    child: ListTile(
                      ...
                      trailing: IconButton(
                        ...
                        onPressed: () => c.speak(c.data[c.qIndex]["en"]!),
                      ),
                    ),
                  ),
                  const SizedBox(height: 24),
                  // الخيارات (كلمات إنكليزية)
                  Expanded(
